...
//! It lives in kotodex-server's crate because the wordhood gate and the
...
use kotodex_server::services::vndb;
...
async fn profile(
    ...
    let (encounters, total) =
        kotodex_server::ingest::profile_script(knowledge, dict_path, work, lines).await?;
