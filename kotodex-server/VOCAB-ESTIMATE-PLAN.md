...
`kotodex-server/examples/` binary that prints the table — the same shape as
...
1. `kotodex-server/examples/vocab_estimate.rs` — prints the bound pair, the per-band
   table, and the same numbers computed per work. **Run the per-work comparison
   and read it.** Stop here if the curves diverge.
2. Move the computation into `kotodex-server/src/services/` once the shape is
   settled. Query helpers stay in kotodex-server until a second consumer appears,
   per CLAUDE.md.
...
5. A panel under `kotodex-server/static/panels/`. Registration is an import plus a
