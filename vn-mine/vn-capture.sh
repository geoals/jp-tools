...
# of the last Japanese clipboard line from Textractor, end = silero-VAD end of
...
VAD_SCRIPT="$(dirname "$(readlink -f "$0")")/vn-vad.py"
...
# === SCREENSHOT (first — capture the window state at the moment of the press) ===
...
read -r SKIP_BYTES LEN_BYTES CLIP_START <<<"$(echo "$SEG_SNAPSHOT" | awk \
  ...
  END {
    ...
    start = line_ts - pre
    if (start < stream_start) { print "STALE"; exit }
...
case "$SKIP_BYTES" in
STALE) die "Last hooked line is older than the 300s audio buffer" ;;
...
if [ -x "$VAD_PYTHON" ] && [ -f "$VAD_SCRIPT" ]; then
  ...
  VAD_OUT=$("$VAD_PYTHON" "$VAD_SCRIPT" "$TMP/vad.wav" 2>/dev/null)
  if [ "$VAD_OUT" != "none" ] && [ -n "$VAD_OUT" ]; then
